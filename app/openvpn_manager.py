...
def _has_admin_rights() -> bool:
    """Return True when the current process can manage Windows services."""
    if os.name != "nt":
        return True
    try:
...
class OpenVPNManager:
    ...
    def _ensure_interactive_service(self) -> None:
        ...
        if not _has_admin_rights():
            message = (
                f"{service_name} service is not running and administrative rights are required for OpenVPN to configure networking. "
                "Start the service manually from Services.msc or run this tool elevated."
            )
