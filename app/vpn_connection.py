...
class VPNConnection:
    ...
    def connect(self) -> Dict[str, object]:
        ...
        try:
            ...
            self._logger.error("Connection aborted: %s", exc)
            status["error"] = str(exc)
            status["requires_admin"] = True
        except Exception as exc:  # noqa: BLE001
            self._logger.exception("Connection routine failed: %s", exc)
            status["error"] = str(exc)
        finally:
            if started_profile and not status.get("is_connected"):
                try:
                    self._manager.stop_all()
                    cleanup_performed = True
                except Exception as cleanup_exc:  # pragma: no cover - defensive cleanup
                    cleanup_error = str(cleanup_exc)
                    self._logger.debug("Cleanup after failed connect attempt failed: %s", cleanup_exc)
            status["cleanup_performed"] = cleanup_performed
            if cleanup_error:
